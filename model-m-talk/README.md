...
An anime-inspired Reveal.js deck for **“That Time We Turned an IBM Model M into a MIDI Instrument to Meet Mark Rober.”** The primary 59-slide keyboard-keyboard cut uses a fixed 1600 × 900 canvas, runs without a build step, and includes speaker notes on every slide. Earlier discovery-led and technical-first cuts are preserved under `archive/`.
...
- Title photo composite: `assets/characters/title-mark-rober.webp`
...
`assets/characters/faces/mio/*.png`, `assets/characters/faces/ren/*.png`, and `assets/characters/faces/kaori/*.png` are 86 original, text-free reaction cutouts generated with OpenAI's built-in image generation tool. Mio (美緒) has long espresso-brown waves with smoky-lavender highlights, gray-violet eyes, a dusty-pink T-shirt, straight-leg light jeans, white canvas sneakers, and restrained gyaru-influenced accessories; her six expression families cover confidence, confusion, dread, realization, amazement, and defeat. Ren (蓮) has warm medium-brown skin, navy hair in a loose low ponytail, amber-brown eyes, a brick-red T-shirt, an open warm off-white button-up, and a muted-cyan strap; Ren's nine reusable reaction families cover immediate understanding, happiness, sadness, exhaustion, annoyance, frustration, fury, unhinged comedy, and harmless knocked-out defeat. Those established Mio and Ren families have restrained base images plus `-big` and `-max` exaggeration levels; each character also has standalone mischievous-confidence, maniacal, and evil reactions. Mio and Ren additionally share six standalone reactions adapted from Kaori's acting library: scandalized, teasing-wink, flustered, pouting, pleading, and ugly-crying. Kaori (香織) is the cast's artistic north star: a coral-pink-haired fantasy heroine, theatrical skeptic, and gyaru-manga reaction specialist with 22 distinct poses spanning teasing, confidence, embarrassment, adoration, judgment, rage, confusion, exhaustion, defeat, maximal crying, mischievous confidence, maniacal delight, and quiet evil. These names identify the characters without assigning an instrument or fixed story role. Prompt set summary: polished anime/manga cel rendering aligned to Kaori, with fine confident dark linework, rich cel contrast, layered hard-edged shadows, controlled blush, glossy hair and eye highlights, dimensional anatomy, dynamic silhouettes, and expressive squash-and-stretch acting; no text, speech bubbles, logos, franchise characters, watermarks, or pale sticker borders. Each Mio and Ren reaction used its previous asset only for pose and emotional intensity, while the approved redesign reference fixed identity and clothing. The images were generated against a flat chroma-key field, converted to alpha locally with color-spill cleanup, edge-checked, and trimmed to visible bounds. See `assets/characters/faces/README.md` for the catalog and naming convention.

`assets/characters/poses/{kaori,mio,ren}/*.png` are 18 matching full-body cutouts generated with OpenAI's built-in image generation tool: neutral, presenting, thinking, and celebrating poses for each character, plus six scene-specific Model M interlude poses. Kaori's established reaction art supplied the shared linework, cel-shadow, anatomy, and acting reference; the approved Mio and Ren redesigns fixed their identities and wardrobes. Prompt set summary: one complete head-to-shoes silhouette, presentation-friendly gesture, exact recurring-character costume, polished Kaori-aligned anime/manga rendering, and no text, border, cast shadow, environment, or crop. Mio's interlude sequence progresses from lovestruck sparkle, heavy blush, and alarm into an arms-folded pout that looks away while sneaking a glance back at Ren; Ren carries relaxed senpai confidence with a raised screwdriver. Two interlude poses intentionally include a screwdriver. The assets were isolated locally into clean alpha cutouts and edge-checked. See `assets/characters/poses/README.md` for the pose catalog.
...
`assets/keyboard/keyboard-keyboard-cutout.png` is a transparent, closely cropped isolation of the presenter-supplied workshop photograph of the finished Lexmark Model M. OpenAI's built-in image editing removed the desk, cables, control box, circuit boards, loose keycaps, and other surrounding equipment while preserving the keyboard's photographed perspective, case, knobs, display, and keycaps. A uniform chroma background was converted to alpha locally for the comic interlude slide.
...
`assets/electronics-tips/daisy-seed-rev4.webp` is a cropped and optimized derivative of the Daisy Seed Rev4 product photograph on the first page of Electrosmith's official Daisy Seed datasheet v1.2.0: `https://daisy.nyc3.cdn.digitaloceanspaces.com/products/seed/Daisy_Seed_datasheet.pdf`. It identifies the actual embedded module used by the project. Attribution does not itself grant reuse permission; confirm permission before redistributing the deck.
